...
# validates: it is written ONCE by protocol_params_mint into a UTxO and
# thereafter only READ (as a reference input) by programmable_logic_base
# and the delegates it dispatches to. The Lean theorems quantify over a
...
#
# STALE CLOSURE MODEL (declared here, not silent — identity-layer step,
# 2026-08): the CLOSURE CHECKS and MANIFEST SCHEMA below were written
# against the deployment shape that existed BEFORE #110 (PLG dissolution:
# programmable_logic_base now dispatches directly to three standalone
# delegates — `transfer`, `third_party`, `unfracking` — instead of
# forwarding through a single `programmable_logic_global` coordinator)
# and BEFORE #99 (federated in-place upgradability: the params UTxO now
# lives at the `coordination_spend` address and is spendable under
# `upgrade_multisig`/`upgrade_cred` authority, not locked at an
# always-fail address; `ProgrammableLogicGlobalParams` grew
# `transfer_cred`, `third_party_cred`, `upgrade_cred` and
# `max_inline_datum_bytes` fields alongside the original three). The
# checks below still run and are internally self-consistent, but they do
# NOT yet model `transfer_cred` / `third_party_cred` / `upgrade_cred` /
# `max_inline_datum_bytes` / `coordination_spend` / `upgrade_multisig` —
# closure over those edges is UNCHECKED, not merely unverified. Extending
# the schema to the current shape is a deliberate follow-up (it changes
# what "closed" means, not just names), tracked separately from this
# step's flat-identity work.
#
...
# `programmable_logic_global.programmable_logic_global.withdraw` dissolved
# (#110) into three standalone delegates; `transfer` and `third_party`
# join `unfracking` here so a further rename/removal of any of the three
# still trips this sanity gate.
for t in \
...
ok "blueprint still declares all six closure validators"
...
# [STALE MODEL — see header] Soft consistency: PLB's stake_cred parameter
# should be the PLG hash (PLB forwards to PLG's withdraw-0). This is
# advisory, not a hard closure edge, since PLB's stake_cred is nominally
# free. Post-#110, PLB no longer takes a stake_cred parameter at all — it
# is parameterised by the protocol-params NFT policy and dispatches PER
# REDEEMER to whichever of `transfer_cred` / `third_party_cred` /
# `unfracking_cred` the coordination datum names, so this manifest field
# and check no longer correspond to anything PLB does. Left running
# (advisory only, never a hard `red`) rather than deleted, so a manifest
# author is not misled into thinking it is meaningful; a real replacement
# needs the schema extension noted at the top of this file.
