...
import { CheckCircle, Plus, Minus, ExternalLink } from "lucide-react";
...
type WhitelistStep = "form" | "signing" | "success";

export function WhitelistSection({ tokens, adminAddress }: WhitelistSectionProps) {
  const { wallet } = useWallet();
  const { toast: showToast } = useToast();
  const { selectedVersion } = useProtocolVersion();
  const network = process.env.NEXT_PUBLIC_NETWORK || "preview";

  // Filter tokens where user has WHITELIST_MANAGER role
  // Also include tokens with ISSUER_ADMIN where substandardId is whitelist
  const manageableTokens = tokens.filter(
    (t) =>
      t.roles.includes("WHITELIST_MANAGER") ||
  ...
  const [step, setStep] = useState<WhitelistStep>("form");
  ...
  const validateForm = (): boolean => {
    ...
    if (!targetAddress.trim()) {
      newErrors.targetAddress = "Address or credential is required";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast({
        title: `${action === "add" ? "Added to" : "Removed from"} Whitelist`,
        description: `Successfully ${action === "add" ? "whitelisted" : "removed from whitelist"} the address`,
        variant: "success",
      });
    } catch (error) {
      console.error("Whitelist error:", error);

      let errorMessage = `Failed to ${action} address ${action === "add" ? "to" : "from"} whitelist`;
  ...
  if (manageableTokens.length === 0) {
    return (
      <div className="flex flex-col items-center py-12 px-6">
        <CheckCircle className="h-16 w-16 text-dark-600 mb-4" />
        <h3 className="text-lg font-semibold text-white mb-2">
          No Whitelist Management Access
        </h3>
        <p className="text-sm text-dark-400 text-center">
          You don&apos;t have whitelist manager permissions for any tokens.
        </p>
      </div>
    );
  }
  ...
  if (step === "success" && txHash) {
    return (
      <div className="flex flex-col items-center py-8">
        ...
        <h3 className="text-lg font-semibold text-white mb-2">
          {action === "add" ? "Address Whitelisted" : "Address Removed"}
        </h3>
        <p className="text-sm text-dark-400 text-center mb-4">
          Successfully {action === "add" ? "added to" : "removed from"} the whitelist
        </p>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {/* Target Address */}
      <Input
        label="Target Address or Credential"
        value={targetAddress}
        onChange={(e) => {
        ...
        helperText={
          action === "add"
            ? "Address to add to the whitelist (KYC approved)"
            : "Address to remove from the whitelist (revoke KYC)"
        }
      />
