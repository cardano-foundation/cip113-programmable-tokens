...
(`lib/multisig.ak:139-145`) requires:
...
- every `AtLeast` has `1 <= required <= length(scripts)`, and its count means
  **distinct parties**: no child may be satisfiable without producing a
  credential, and no credential may appear under two children, so one signature
  cannot raise the count twice (`lib/multisig.ak:205-215`);
- **no tree is satisfiable without producing a credential**. A validity range is
  evidence of when, never of who, so a tree that only ever asks for one is an
  authority anyone can exercise (`lib/multisig.ak:154-165`);
- the whole tree fits under `max_size` (`lib/multisig.ak:118`), so an authority
