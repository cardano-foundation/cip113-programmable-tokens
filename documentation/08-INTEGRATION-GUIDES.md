...
| **Administrative** | `SpendViaThirdParty` → `third_party` (`ThirdPartyRedeemer { ... }`) | Admin-authorized (forced transfer / seizure / burn). No owner signature required. The continuing output preserves the holder's address and datum; only the subject policy's tokens change — every token of that policy is reachable, companion assets included (see [`03-CONTROL-SCOPE-AND-ADMIN-AUTHORITY.md`](./03-CONTROL-SCOPE-AND-ADMIN-AUTHORITY.md) §2.2). |
